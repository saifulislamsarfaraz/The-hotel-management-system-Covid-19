from employeeRec.models import Employee, Detected
import cv2
import pickle
...
def identify1(frame, name, buf, buf_length, known_conf):
    ...
    if count >= known_conf:
        ...
        cache[name] = 'detected'
        path = 'employeeRec/facerec/detected/{}_{}.jpg'.format(name, timestamp)
...
def identify_faces(video_capture):
    ...
    while True:
        ...
        if process_this_frame:
            predictions = predict(rgb_frame, model_path="employeeRec/facerec/models/trained_model.clf")
            # print(predictions)
